...
iOS registers the `.lxbp` document type and receives file URLs through its scene delegate. Android accepts open and single-file share intents for the custom, JSON, and generic binary MIME types, then checks the document name for the `.lxbp` extension. Each platform reads the file and passes its contents through a method channel, buffering a cold-launch file until Flutter is ready. The same guarded decoder used by the in-app picker validates the contents before opening the later creation steps.
